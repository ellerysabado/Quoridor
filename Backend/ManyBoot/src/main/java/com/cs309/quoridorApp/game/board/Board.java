...
import com.cs309.quoridorApp.game.board.tileBound.SingleWall;
...
public class Board
{
    ...
    public boolean addWall(Player player, int x1, int y1, int x2, int y2)
    {
        if(player != null && withinBoard(x1, y1) && withinBoard(x2, y2))
        {
            Tile t1 = tiles[x1][y1];
            Tile t2 = tiles[x2][y2];
            SingleWall singleWall = new SingleWall(player);

            return singleWall.setTile(t1, t2);
        }
    ...
    public int[][] getBytes()
    {
        int[][] toReturn = new int[getRowSize()][getColumnSize()];
        for(int y = 0; y < getColumnSize(); y++)
        {
            for(int x = 0; x < getRowSize(); x++)
            {
    ...
    {
        return o instanceof Board && ((Board)o).game.equals(this.game);
    }
}
