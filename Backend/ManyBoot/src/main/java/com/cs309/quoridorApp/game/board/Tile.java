package com.cs309.quoridorApp.game.board;

import com.cs309.quoridorApp.game.board.tileBound.Building;
import com.cs309.quoridorApp.game.board.tileBound.Entity;
import com.cs309.quoridorApp.game.board.tileBound.PlayerMarker;
import com.cs309.quoridorApp.game.board.tileBound.TileBound;
...
public class Tile {
    ...
    public boolean canAddOnlyBuildings(Entity entity, Direction dir)
    {
        for(Building b: buildings)
            if(b != null && b.preventsMovement(entity, this, dir))
                return false;
    ...
    public boolean canAddOnlyEntities(Entity entity, Direction dir)
    {
        for(Entity e: entities)
            if(e != null && e.preventsMovement(entity, this, dir))
                return false;
    ...
    public String[] separateStrings()
    {
        ...
        toReturn[0] = getBuilding(Direction.UP) == null ? "     " : "_____";
        toReturn[1] = (getBuilding(Direction.LEFT) == null ? " " : "[") +
                getEntities().size() + " " + x + " " + y +
                (getBuilding(Direction.RIGHT) == null ? " " : "]");
        toReturn[2] = getBuilding(Direction.DOWN) == null ? "     " : "-----";
        return toReturn;
    }
    ...
    public byte getByteFormat()
    {
        ...
        for(Entity entity: getEntities())
        {
            if(!player && entity instanceof PlayerMarker) {
                player = true;
                toReturn += (board.game.getClientList().getTeam(((PlayerMarker) entity).getPlayer()) + 1) << 4;
            }
    ...
    /**
     * To get the Row of the tile
     * @return x, which is the row of the tile
     */
    public int getRow() {
        return y;
    }
    /**
     * To get the col of the tile
     * @return y, which is the col of the tile
     */
    public int getCol() {
        return x;
    }
    ...
    public enum Direction
    {
        ...
        public static List<Direction> getNonNone()
        {
    ...
    {
        return o instanceof Tile && ((Tile)o).x == this.x && ((Tile)o).y == this.y && ((Tile)o).board.equals(board);
    }
}
